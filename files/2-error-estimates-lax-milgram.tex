...
\end{theorem}
\begin{proof}
  From Ceà's lemma~\ref{lemma:cea} and the interpolation error estimate (Theorem~\ref{theo:interpolation_error}), we have that
...
\end{equation}
From now on we consider $p=2$ and use the notation $\norm{u}_k$ to mean $\norm{u}_{k,2,\Omega}$. Let $(\cdot)^T$ be the adjoint operator:
\begin{definition}
Let $V \subset H^s(\Omega)$, with $\|\cdot\|_V \sim \|\cdot\|_s$ and $A \in \L(V,V')$. We say that $A$ (or $A^T$) is $r$-\emph{regular} if there exists an interval $[s_{min}, s_{max}]$ such that $\forall f,g \in H^\ell(\Omega)$, with $\ell \in [s_{min}, s_{max}]$, the problem $Au=f$ (or $A^T\varphi = g)$ admits a unique solution $u$ (or $\varphi$) which satisfies
\begin{gather*}
\norm{u}_{\ell+r} \lesssim \norm{f}_\ell \\
\norm{\varphi}_{\ell+r} \lesssim \norm{g}_\ell
\end{gather*}
\end{definition}
For finite element spaces of order $k$, inequality~\eqref{ineq:cea_scaling2}, combined with the assumption of $r$-regularity, becomes
\rev{Check the exponents after my fix, i was not convinced by $s+r-k$ on the second inequality, that one should be obtained \emph{after} the Nietzsche trick.}
\begin{equation}\label{eq:a_priori_error_estimates}
\norm{u-u_h}_s \lesssim h^{\ell+r-s} \abs{u}_{\ell+r} \lesssim h^{\ell+r-s} \norm{f}_\ell \quad  0 \le \ell+r \le k + 1.
\end{equation}
This is generally the \emph{a-priori} error estimate we were looking for. However, as said before, we may not be allowed to use all the values for $k$ in the range above, as the following example will show.

\begin{example}
The Poisson problem on a Lipschitz, convex domain is $2$-regular (i.e. $r = 2$), with $s_{min} = -1$ and $s_{max}=0$. If $u \in H^1_0(\Omega)$ (i.e. $s = 1$) the inequality~\eqref{eq:a_priori_error_estimates} becomes
\[
...
\begin{remark}
With the hypotheses above, if $f \in V$, by the Hahn-Banach theorem $f$ can be extended to a functional $\tilde{f} \in V'$, i.e.
\[
