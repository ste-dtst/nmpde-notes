...
Consider an affine mapping from a reference triangle $\hat T$, to a generic triangle $T_m$:
\begin{align}
F_m: &\hat{T} \to T_m \\
...
\[
\hat{\rho} := \sup_{\tilde{B} \subset \hat{T}} \diam(\tilde{B}), \quad \hat{h} := \max_{\hat x, \hat y  \in \hat T}|\hat x - \hat y|,
\]
where $\tilde{B}$ denotes a ball (see Figure~\ref{fig:scaling}).
...
Following the same argument in the opposite direction, we obtain:
\[
