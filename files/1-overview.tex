...
\end{equation}
Let $V \supset V_h = \Span\{v_i\}_{i=1}^n$, with $\dim V_h = n$ (generally speaking the space $V_h$ will be a space of polynomials of some degree).  The space $V_h'$ is usually described in terms of the \emph{covectors}:
\[
V_h' = \Span\{v^i\}_{i=1}^n.
\]
In particular, each $v^i$ is extended via the Hahn-Banach theorem to $V'$, in order to see $V_h'$ as a subspace of $V'$. Let's call the Hahn-Banach extensions $\tilde{v}^i$. With this notation, each $v\tilde{v}^i$ is the dual vector of $v_i$, i.e.
\[
\tilde{v}^i(v_j) = \delta_{ij}.
\]
In particular, if $u_h\in V_h$, we have
\[
u_h = \sum_{i=1}^n u^i v_i = \sum_{i=1}^n \tilde{v}^i(u_h) v_i.
\]
This leads to the natural definition of the \emph{interpolation operator} in $V_h$:
\begin{align}
\Pi_{V_h}: &V \to V_h \\
& u \mapsto \sum_{i=1}^n \tilde{v}^i(u) v_i = \sum_{i=1}^n u^i v_i.
\end{align}

\begin{remark}
$\Pi_{V_h}$ is a projection onto $V_h$. In fact:
\begin{align}
\Pi_{V_h}(\Pi_{V_h}(u)) &= \tilde{v}^i (u^j  v_j) v_i \\
&= u^j \tilde{v}^i(v_j) v_i \\
&= \delta_{ij} u^j  v_i \\
&= u^i  v_i.
\end{align}
\end{remark}
