...
Once we have defined a triangulation and chosen a finite element type, we need to distribute and enumerate the degrees of freedom (DoFs) on the mesh. This process establishes a relationship between local DoFs on each element and their global indices in the resulting finite dimensional space.

\subsubsection{Degrees of freedom per object}

A fundamental concept in DoF enumeration is the ``degrees of freedom per object'' vector ($\dpo$). This vector is attached to a finite element and specifies how many DoFs are associated with each type of topological entity in the mesh:
...
For each element, we then establish a mapping between local DoF indices and global indices. This mapping determines the relation between the local enumeration of DoFs and the global one.

For an element $T$, this mapping can be represented as an array:
\[
\text{dof\_indices}_{T}[i] = \text{global index of local DoF}~i \text{ on element } T
\]

This mapping is constructed during DoF enumeration by:

\begin{enumerate}
  \item Identifying all the topological entities of the element (vertices, edges, etc.)
...
A compact way to represent this mapping is to use a permutation tensor for each element $T_m$:
\[
 P_{m~i\alpha} = \begin{cases}
  1 & \text{if the } i\text{-th DoF of element } T_m \text{ is the } \alpha\text{-th global DoF}, \\
  0 & \text{otherwise}.
 \end{cases}
\]
...
\begin{itemize}
  \item Element $T_1$: $\text{dof\_indices}_{T_1} = [0, 1, 3, 4,5,6]$
  \item Element $T_2$: $\text{dof\_indices}_{T_2} = [1, 2, 3, 7, 8, 5]$
\end{itemize}

This mapping ensures that the support points associated to the shared vertices and to the shared edge between elements are assigned the same global indices. Such a choice ensures global continuity of the finite element space, i.e., the finite element space is continuous across the entire triangulation.