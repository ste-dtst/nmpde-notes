...
\begin{figure}[t]
  ...
  \caption{The domain $\Omega$ and its boundary $\partial\Omega$. We generally indicate with $n$ the outer normal to the boundary.}
  \label{fig:domain}
\end{figure}
...
\[
\|u\|_{L^p(\Omega)} := \left(\int_\Omega |u(x)|^p \, dx \right)^{1/p} < \infty.
\]
...
\rev{I am a bit uncertain about the last sentence: this is very close to the matrix we would get with second order centered finite differences... but here we got $h$ instead of $h^2$.}
